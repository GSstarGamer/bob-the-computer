...
from bob.stage1.models import GitRemote, GitSnapshot
...
class Stage1GitError(RuntimeError):
...
def _run_git(repo_path: Path, *args: str, check: bool = True) -> str:
    ...
    if check and completed.returncode != 0:
        raise Stage1GitError(completed.stderr.strip() or completed.stdout.strip() or "git command failed")
    return completed.stdout.strip()


def snapshot_repo(repo_path: Path) -> GitSnapshot:
    if not repo_path.exists():
        raise Stage1GitError(f"Repository path does not exist: {repo_path}")
    if not repo_path.is_dir():
        raise Stage1GitError(f"Repository path is not a directory: {repo_path}")
...
def ensure_clean_repo(snapshot: GitSnapshot) -> None:
    if snapshot.is_dirty:
        raise Stage1GitError(
            "Stage 1 requires a clean checkout before Codex write mode. "
            f"Dirty entries: {', '.join(snapshot.status_lines[:10])}"
        )


def build_repo_snapshot_text(repo_path: Path, snapshot: GitSnapshot) -> str:
    lines = [
...
def diff_summary(repo_path: Path) -> list[str]:
    output = _run_git(repo_path, "diff", "--stat", check=False)
    if not output:
        return []
    return [line.strip() for line in output.splitlines() if line.strip()]


def diff_name_only(repo_path: Path) -> list[str]:
    output = _run_git(repo_path, "diff", "--name-only", check=False)
    return [line.strip() for line in output.splitlines() if line.strip()]
